...
public class Bomb extends Weapon {
    ...
    public void use(Robot robot, GameManager gameManager, BoardManager boardManager, InventoryManager inventoryManager) {
        int posX = robot.getX();
        int posY = robot.getY();
        for (int x = max(posX - getRange(), 0); x <= min(posX + getRange(), boardManager.getWidth() - 1); x++) {
            int dx = abs(x - posX);
            for (int y = max(posY - getRange(), 0); y <= min(posY + getRange(), boardManager.getHeight() - 1); y++) {
                int dy = abs(y - posY);
                int damage = (int) round(getDamage() * pow((1 - getPiercingLoss()), sqrt(dx * dx + dy * dy)));
                if (damage > 0) {
