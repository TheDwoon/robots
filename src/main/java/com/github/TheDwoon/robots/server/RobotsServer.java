...
//        gameManager.spawnAi(new RepeatingAI(DriveForward.INSTANCE));
        ...
        gameManager.spawnItemsPlaced(IntStream.rangeClosed(4, 10).mapToObj(i -> new Gun(i, 3)).toArray(Item[]::new));
    ...
    private GameManager loadGame(final String mapName) {
        ...
        try {
            boardManager = MapFileParser.parseBoard(getClass().getResourceAsStream("/map/" + mapName + ".map"));
